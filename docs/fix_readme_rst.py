...
def fix_readme_rst(file_path: Path) -> None:
    ...
    # Fix screenshot path: change ./screenshot.png to ../../screenshot.png
    # (from docs/source/readme.rst, need to go up 2 levels to project root)
    content = re.sub(
        r'\.\. image:: \./screenshot\.png',
        '.. image:: ../../screenshot.png',
        content,
    )
    content = re.sub(
        r':target: \./screenshot\.png', ':target: ../../screenshot.png', content
    )

    # Fix broken link for cookiecutter template
    content = re.sub(
        r'`APRSD Extension Python Cookiecutter <>`_',
        '`APRSD Extension Python Cookiecutter <https://github.com/hemna/cookiecutter-aprsd-extension>`_',
        content,
    )

    # Fix duplicate Features label - rename second occurrence
    # The second "Features" appears as a subsection (^^^) under "Accessing the Web Interface"
    # Replace it with "WebChat Features" to avoid duplicate label warning
    lines = content.split('\n')
    fixed_content_lines = []
    features_found = 0
    i = 0
    ...
    while i < len(lines):
        ...
        if i > 0 and lines[i-1].strip() == 'Features':
            features_found += 1
            # If this is the second occurrence and it's a subsection (^^^), rename it
            if features_found == 2 and line.strip() and line.strip()[0] == '^':
                fixed_content_lines[-1] = 'WebChat Features'
                # Keep the underline but adjust length
                fixed_content_lines.append('^' * len('WebChat Features'))
                i += 1
                continue

        fixed_content_lines.append(line)
        i += 1
